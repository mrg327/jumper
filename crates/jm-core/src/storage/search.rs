...
impl SearchEngine {
    ...
    fn collect_files(&self, filter: &SearchFilter) -> Vec<PathBuf> {
        ...
        // Issue files
        let issues_dir = self.data_dir.join("issues");
        if filter.file_type.is_none() || filter.file_type.as_deref() == Some("issue") {
            if issues_dir.exists() {
                let mut issue_files: Vec<PathBuf> = fs::read_dir(&issues_dir)
                    .into_iter()
                    .flatten()
                    .flatten()
                    .map(|e| e.path())
    ...
    fn search_file(&self, file_path: &Path, filter: &SearchFilter) -> Vec<SearchResult> {
        ...
        let project_slug = if file_type == "project" || file_type == "issue" {
            file_path
                .file_stem()
                .and_then(|s| s.to_str())
                .unwrap_or("")
    ...
    fn get_file_type(&self, file_path: &Path) -> String {
        if file_path.starts_with(self.data_dir.join("projects")) {
            ...
            "journal".to_string()
        } else if file_path.starts_with(self.data_dir.join("issues")) {
            "issue".to_string()
