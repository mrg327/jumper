...
use crate::text_utils::{char_to_byte_idx, next_word_boundary, prev_word_boundary, wrapped_cursor_position};
use crate::theme;
...
fn render_input_box(state: &SwitchState, frame: &mut Frame, area: Rect) {
    ...
    // Position the terminal cursor so it blinks at the correct wrapped location.
    let inner_width = inner.width as usize;
    if inner_width > 0 {
        let (crow, ccol) = wrapped_cursor_position(buf, inner_width, pos);
        let cursor_x = inner.x + ccol;
        let cursor_y = inner.y + crow;
