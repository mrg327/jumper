...
use crate::text_utils::char_to_byte_idx;
use crate::theme;
...
impl InputModal {
    ...
    pub fn render(&self, frame: &mut Frame, area: Rect) {
        let popup_area = crate::modals::centered_rect(60, 30, area);
        ...
        // Split inner area: prompt row, input row, spacer, footer.
        ...
        // Input line with cursor marker.
        let input_line = build_input_line(&self.input, self.cursor_pos);
        frame.render_widget(Paragraph::new(input_line), input_area);

        // Position the actual terminal cursor so it blinks.
        let char_x = self.cursor_pos.min(self.input.chars().count());
        let cursor_x = input_area.x + char_x as u16;
